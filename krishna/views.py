...
def homepage(request):
    ...
    if request.method =="POST":
        try:
            ...
            data = {'rooms':room,'all_location':all_location,'flag':True,'hotels':hotels,'rooms':room}
            print(hotels[0].rooms)
            response = render(request,'index.html',data)
        except Exception as e:
            messages.error(request,e)
            hotels = Hotels.objects.all()            
            room = Rooms.objects.all()
            response = render(request,'index.html',{'all_location':all_location,'hotels':hotels,'rooms':room})
        ...
        print(hotels[0].rooms)
        room = Rooms.objects.all()
        data = {'all_location':all_location,'hotels':hotels,'rooms':room}
        response = render(request,'index.html',data)
...
#     user = request.user
#     data = Reservation.objects.all().filter(guest = user)[0]
#     print(data.room)
#     print(data.check_in)
#     buf = io.BytesIO()
...
#     lines = ['Booking Successfull !!',f'Name : {data.guest}',f'CheckIN : {data.check_in} and CheckOUT : {data.check_out}',f'room {data.room}']
#     for line in lines:
...
#     c.save()
#     buf.seek(0)
#     return FileResponse(buf,as_attachment=True,filename='demo.pdf')
    #return  render(request, 'user/mybooking.html', {'pdf_url': response.url})
def display_pdf(request):
    # user = request.user
    # data = Reservation.objects.all().filter(guest=user)[0]
    ...
    # buf = io.BytesIO()
    # c = canvas.Canvas(buf, pagesize=letter, bottomup=0)
    # c.setTitle("Homestay - Booking Confirmation")  # Set PDF title

    # # Set font styles
    # c.setFont("Helvetica-Bold", 18)  # Title font
    # c.setFont("Helvetica", 14)  # Content font

    # # Title
    # c.drawCentredString(4.25 * inch, 10 * inch, "Homestay - Booking Confirmation")

    # # Content
    # lines = [
    #     'Booking Successful!',
    #     f'Name: {data.guest}',
    #     f'Check-In: {data.check_in} and Check-Out: {data.check_out}',
    #     f'Room: {data.room}'
    # ]
    ...
    # c.save()
    # buf.seek(0)
    # return FileResponse(buf, as_attachment=True, filename='booking_confirmation.pdf')
    



    user = request.user
    data = Reservation.objects.filter(guest=user).first()

    if data is None:
        return HttpResponse("No booking data found.")
    ...
    # Content
    c.drawString(1 * inch, 9 * inch, f'Dear {data.guest.username},')
    c.drawString(1 * inch, 8.5 * inch, 'We are pleased to confirm your booking with Homestay.')
    c.drawString(1 * inch, 8 * inch, 'Booking Details:')
    c.drawString(1 * inch, 7.5 * inch, f'Check-In Date: {data.check_in.strftime("%Y-%m-%d")}')
    c.drawString(1 * inch, 7 * inch, f'Check-Out Date: {data.check_out.strftime("%Y-%m-%d")}')
    c.drawString(1 * inch, 6.5 * inch, f'Room: {data.room}')
    c.drawString(1 * inch, 6 * inch, 'Please note the following details:')
    c.drawString(1 * inch, 5.5 * inch, '- Check-in time: 3:00 PM')
    c.drawString(1 * inch, 5 * inch, '- Check-out time: 12:00 PM')
    c.drawString(1 * inch, 4.5 * inch, 'If you have any questions or need further assistance,')
    c.drawString(1 * inch, 4 * inch, 'please do not hesitate to contact our customer service.')
    c.drawString(1 * inch, 3.5 * inch, 'Thank you for choosing Homestay, and we look forward to')
    c.drawString(1 * inch, 3 * inch, 'providing you with an enjoyable stay.')
    c.drawString(1 * inch, 2.5 * inch, 'Best regards,')
    c.drawString(1 * inch, 2 * inch, 'Homestay Management')
